...
public class LoadRequestsController : ControllerBase
{
    ...
    /// <summary>
    /// Get parts summary for a specific load request (LoadRequestId, ProductId, PartId, RequiredQuantity)
    /// </summary>
    /// <param name="id">Load request ID (bigint)</param>
    /// <returns>List of parts summary</returns>
    [HttpGet("{id}/parts")]
    [HttpGet("parts/by-load-request/{id}")]
    [RequirePermission("LOAD_REQUESTS", PermissionAction.View)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<APIBaseResponse<IEnumerable<LoadRequestPartSummaryDto>>>> GetPartsByLoadRequestId([FromRoute] long id)
    {
        _logger.LogInformation("User {User} fetching parts for load request: {LoadRequestId}", User.Identity?.Name, id);

        var query = new CleanSample.Application.Queries.LoadRequestPart.GetLoadRequestPartsByLoadRequestIdQuery(id);
        var result = await _mediator.Send(query);
