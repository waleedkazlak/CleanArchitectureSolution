...
public class PickSearchFilterDtoValidator : AbstractValidator<PickSearchFilterDto>
{
    private readonly string[] _validSortFields = { "PickDate", "PickRequestId", "PartId", "Quantity", "Status", "CreatedAt" };
    private readonly string[] _validSortDirections = { "asc", "desc" };

    public PickSearchFilterDtoValidator()
    {
        ...
        RuleFor(x => x.PickRequestPartId)
            .GreaterThan(0).WithMessage("Pick request part ID must be greater than 0")
            .When(x => x.PickRequestPartId.HasValue);
