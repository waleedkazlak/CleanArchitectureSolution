...
public class GetFieldAssembliesByProductVariantIdQueryHandler : IRequestHandler<GetFieldAssembliesByProductVariantIdQuery, List<FieldAssemblyDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetFieldAssembliesByProductVariantIdQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<List<FieldAssemblyDto>> Handle(GetFieldAssembliesByProductVariantIdQuery request, CancellationToken cancellationToken)
    {
        var assemblies = await _unitOfWork.FieldAssemblies.GetByProductVariantIdAsync(request.ProductVariantId);
