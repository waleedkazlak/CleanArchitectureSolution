...
public class GetVehicleLoadsByVehicleIdQueryHandler : IRequestHandler<GetVehicleLoadsByVehicleIdQuery, IEnumerable<VehicleLoadDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetVehicleLoadsByVehicleIdQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<IEnumerable<VehicleLoadDto>> Handle(GetVehicleLoadsByVehicleIdQuery request, CancellationToken cancellationToken)
    {
        var loads = await _unitOfWork.VehicleLoads.GetByVehicleIdAsync(request.VehicleId);
