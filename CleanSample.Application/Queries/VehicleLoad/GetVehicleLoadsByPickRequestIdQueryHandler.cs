...
public class GetVehicleLoadsByPickRequestIdQueryHandler : IRequestHandler<GetVehicleLoadsByPickRequestIdQuery, IEnumerable<VehicleLoadDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetVehicleLoadsByPickRequestIdQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<IEnumerable<VehicleLoadDto>> Handle(GetVehicleLoadsByPickRequestIdQuery request, CancellationToken cancellationToken)
    {
        var loads = await _unitOfWork.VehicleLoads.GetByPickRequestIdAsync(request.PickRequestId);
