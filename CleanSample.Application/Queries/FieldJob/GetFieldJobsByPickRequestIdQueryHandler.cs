...
public class GetFieldJobsByPickRequestIdQueryHandler : IRequestHandler<GetFieldJobsByPickRequestIdQuery, List<FieldJobDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetFieldJobsByPickRequestIdQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<List<FieldJobDto>> Handle(GetFieldJobsByPickRequestIdQuery request, CancellationToken cancellationToken)
    {
        var jobs = await _unitOfWork.FieldJobs.GetByPickRequestIdAsync(request.PickRequestId);
